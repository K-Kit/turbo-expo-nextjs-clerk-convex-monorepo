...
export default function ProjectDetailPage(props: {
  ...
  const worksite = useQuery(
    api.worksites.get,
    project?.worksiteId ? { worksiteId: project.worksiteId } : "skip",
  );

  // Get manager details if available
  const manager = useQuery(
    api.users.get,
    project?.managerId ? { userId: project.managerId } : "skip",
  );

  // Get users in the tenant for task assignees
                ...
                {worksite && (
                  <div>
                    ...
                    <div className="flex items-center text-sm">
                      ...
                      <Link
                        href={`/worksites/${project.worksiteId}`}
                    ...
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Link href={`/projects/${params.id}/tasks/${task._id}/edit`}>
                      ...
                      <div className="flex flex-wrap gap-4 mt-3 text-xs text-gray-500">
                        {task.assignedTo && (
                          <div className="flex items-center">
                            ...
                            {(() => {
                              const assignedUser = users?.find((user: any) => user._id === task.assignedTo);
                              return `Assigned to: ${assignedUser ? assignedUser.name : 'Unknown'}`;
                            })()}
                          </div>
                        )}
