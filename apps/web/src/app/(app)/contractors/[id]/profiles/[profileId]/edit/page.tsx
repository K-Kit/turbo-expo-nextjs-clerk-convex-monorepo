...
import { X, Plus, ArrowLeft, Briefcase, Award } from "lucide-react";
...
export default function EditContractorProfilePage() {
  ...
  const profileId = params.profileId as Id<"contractorProfiles">;
  const router = useRouter();
  const { toast } = useToast();
  
  const profile = useQuery(api.contractors.getContractorProfiles, { contractorId })?.find(
    p => p._id === profileId
  );
  const updateProfile = useMutation(api.contractors.updateContractorProfile);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to update contractor profile",
        variant: "destructive"
      });
    } finally {
